...
Status convert_to_arrow_type(const DataTypePtr& origin_type,
    ...
    case TYPE_ARRAY: {
        ...
        RETURN_IF_ERROR(
                convert_to_arrow_type(type_arr->get_nested_type(), &item_type, timezone, datetime_naive));
    ...
    case TYPE_MAP: {
        ...
        RETURN_IF_ERROR(
                convert_to_arrow_type(type_map->get_key_type(), &key_type, timezone, datetime_naive));
        RETURN_IF_ERROR(
                convert_to_arrow_type(type_map->get_value_type(), &val_type, timezone, datetime_naive));
    ...
    case TYPE_STRUCT: {
        ...
        for (size_t i = 0; i < type_struct->get_elements().size(); i++) {
            ...
            RETURN_IF_ERROR(convert_to_arrow_type(type_struct->get_element(i), &field_type, timezone,
                                                  datetime_naive));
            fields.push_back(
...
Status get_arrow_schema_from_block(const Block& block, std::shared_ptr<arrow::Schema>* result,
    ...
    for (const auto& type_and_name : block) {
        ...
        // Backport note: DATETIMEV2 is a wall-clock value, so emit a timezone-naive Arrow
        // timestamp to keep the writer and reader consistent (the reader decodes naive Arrow
        // timestamps in UTC). convert_to_arrow_type's datetime_naive flag applies this to
        // DATETIMEV2 (TIMESTAMPTZ stays aware) and propagates into nested ARRAY/MAP/STRUCT.
        RETURN_IF_ERROR(
                convert_to_arrow_type(type_and_name.type, &arrow_type, timezone, /*datetime_naive=*/true));
...
Status get_arrow_schema_from_expr_ctxs(const VExprContextSPtrs& output_vexpr_ctxs,
    ...
    for (int i = 0; i < output_vexpr_ctxs.size(); i++) {
        ...
        // Same rationale as get_arrow_schema_from_block: emit DATETIMEV2 (including nested) as
        // timezone-naive so the writer and reader agree.
